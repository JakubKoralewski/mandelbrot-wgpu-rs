...
use crate::views::{MandelbrotOnlyView, FractalViewable};

pub mod utils;

use crate::utils::{ABSOLUTE_PATH, WindowSize, Position, Changed};
...
fn main() {
	...
	let (watcher, frag_file_change_receiver, mut view) = MandelbrotOnlyView::new(&device, size);
		...
		match event {
			event::Event::WindowEvent {
				...
				event::WindowEvent::RedrawRequested => {
					...
					queue.lock().unwrap().submit(&[render_buf, fps_buf]);
				...
				event::WindowEvent::Resized(size) => {
					...
					*swap_chain.lock().unwrap() = device.lock().unwrap().create_swap_chain(&surface, &sc_desc);
					...
					let command_buf = view.lock().unwrap().resized(
						&device,
						&window_size
					);
				...
				event::WindowEvent::CursorMoved {
					...
					if is_left_button_pressed && is_cursor_on_screen {
						...
						command_buf =
							view.lock().unwrap().new_position(&device, x, y, true);
						changed.lock().unwrap().set(true, "cursor moved");
					} else {
						command_buf =
							view.lock().unwrap().new_position(&device, x, y, false);
					}
					queue.lock().unwrap().submit(&[command_buf]);
				}
				event::WindowEvent::MouseInput {
					...
					view.lock().unwrap().mouse_input(button, state);
				}
				...
				event::WindowEvent::KeyboardInput {
					...
					match key {
						...
						VirtualKeyCode::Numpad1 | VirtualKeyCode::Key1 => {} //TODO:
						_ => ()
					}
				}
